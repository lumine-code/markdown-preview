...
module.exports = {
  provideBackgroundTips() {
    return {
      ...
      tips: [
        "You can open a live preview of the Markdown file you are editing with {{ 'markdown-preview:toggle' | keystroke }}",
      ],
    };
  },
  ...
  activate() {
    ...
    this.disposables.add(
      new Disposable(() => {
        document.adoptedStyleSheets = document.adoptedStyleSheets.filter(
          (styleSheet) => styleSheet !== this.style,
        );
        this.style = null;
      }),
    );
    ...
    // read from the dispatch target, not the active editor. Cold command
    // selectors are registered individually to match their manifest bytes.
    const previewFile = this.previewFile.bind(this);
    this.disposables.add(
      ...PREVIEWABLE_EXTENSIONS.map((extension) =>
        lumine.commands.add(
  ...
  deactivate() {
    if (MarkdownPreviewView != null) {
      for (const item of lumine.workspace.getPaneItems()) {
        if (!(item instanceof MarkdownPreviewView)) continue;
  ...
  async copyHTML() {
    ...
    const html = await renderer.toHTML(text, editor.getPath(), editor.getGrammar());
  ...
  async exportToPDF() {
    ...
    const { filePath } = await lumine.window.showSaveDialog(view.getPDFSaveDialogOptions());
    if (!filePath) return;
