...
class EditorCache {
  ...
  adoptDocument(document) {
    for (const editor of this.editorsByPre.values()) {
      const element = editor.getElement();
      if (element.ownerDocument !== document) document.adoptNode(element);
    }
  }
}
...
exports.toDOMFragment = async function (
  text,
  filePath,
  grammar,
  editorId,
  ...
  let cache = providedCache || EditorCache.findOrCreateById(editorId);
  cache.beginRender();

  const domFragment = chooseRender(text, filePath, document);
  annotatePreElements(domFragment, defaultLanguage);

  return [
    domFragment,
    async (element) => {
      await highlightCodeBlocks(element, grammar, cache, makeLumineEditorNonInteractive);
      cache.endRender();
    },
  ];
};

exports.toHTML = async function (
  text,
  filePath,
  grammar,
  _editorId,
  ...
  await highlightCodeBlocks(div, grammar, null, convertLumineEditorToStandardElement);
...
function render(text, filePath, document) {
  ...
  html = createDOMPurify(document.defaultView).sanitize(html, {
...
function highlightCodeBlocks(element, grammar, cache, editorCallback) {
  ...
  for (const preElement of element.querySelectorAll("pre")) {
    ...
    const editorGrammar = editor.getBuffer().getLanguageMode().grammar ?? null;
...
function makeLumineEditorNonInteractive(editorElement) {
  editorElement.setAttributeNode(editorElement.ownerDocument.createAttribute("gutter-hidden"));
  ...
  return new Promise(function (resolve) {
    ...
    const done = () => {
      ...
      let proceed = function () {
        // Both paths below can reach this, and the loser must not run: the
        // timeout clears the timer but cannot cancel an already-pending
        // `getNextUpdatePromise`, so a second pass would append every line to
        // the `pre` a second time. Claim the flag before doing any work.
        if (updated) return;
        updated = true;
        clearTimeout(timeout);
        for (const line of editorElement.querySelectorAll(".line:not(.dummy)")) {
          const line2 = editorElement.ownerDocument.createElement("div");
