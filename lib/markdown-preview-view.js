...
module.exports = class MarkdownPreviewView {
  ...
  constructor({ editorId, filePath }) {
    ...
    this.disposables.add(
      lumine.workspace.registerFileDocument({
        owner: this,
        getPath: () => (this.file ? this.filePath : null),
        ...
        endFileOperation: () => {
          this.fileOperationDepth--;
          if (!this.destroyed) return this.renderMarkdown();
        },
      }),
    );
    this.editorCache = new renderer.EditorCache();
    this.renderGeneration = 0;
    this.currentRender = null;
    this.lifetimeAbortController = new AbortController();
  ...
  destroy() {
    ...
    this.destroyed = true;
    this.currentRender?.controller.abort();
  ...
  observeFilePath(filePath) {
    ...
    this.file = file;
    const reconcile = () => {
      if (this.file === file && !this.destroyed && !this.fileOperationDepth) this.renderMarkdown();
    };
    this.fileSubscriptions.add(
      file,
      file.onDidChange(reconcile),
      file.onDidInvalidate(reconcile),
      file.onDidError((error) => console.error("Unable to watch Markdown preview", error)),
    );
  }
      ...
      if (this.editor != null) {
        ...
        this.disposables.add(
          this.editor.onDidDestroy(() => {
            ...
            this.editor = null;
            if (filePath) this.subscribeToFilePath(filePath);
          }),
        );
        this.handleEvents();
        this.renderMarkdown();
  ...
  renderMarkdown() {
    ...
    render.promise = this.renderMarkdownGeneration(render).then(() => {
      // A caller awaiting a render needs the settled DOM even when a grammar,
      // configuration or filesystem event superseded that particular render.
      if (!this.destroyed && this.currentRender !== render) return this.currentRender?.promise;
    });
    return render.promise;
  }

  async renderMarkdownGeneration(render) {
    if (!this.loaded) {
      this.showLoading();
    }
    ...
    let source;
    try {
      source = await this.getMarkdownSource();
    } catch (reason) {
      if (this.isCurrentRender(render)) this.showError({ message: reason });
      return;
    }

    if (!this.isCurrentRender(render) || source == null) return;

    if (!this.loaded) {
      // If we haven't loaded yet, defer before we render the Markdown for the
      // first time. This allows the pane to appear and to display the loading
      // indicator. Otherwise the first render happens before the pane is even
      // visible. This doesn't slow anything down; it just shifts the work
      // around so that the pane appears earlier in the cycle.
      await new Promise((resolve) => setTimeout(resolve, 0));
      if (!this.isCurrentRender(render)) return;
    }

    return this.renderMarkdownText(source, render);
  }

  isCurrentRender(render) {
    return (
      !this.destroyed &&
      !render.controller.signal.aborted &&
      this.currentRender?.generation === render.generation
    );
  }

  getMarkdownSource() {
    if (this.file && this.filePath) {
      // Subscribe before the authoritative read so initial rendering cannot
      // miss a write in an asynchronous watcher-start gap.
      return this.file.ready
        .catch(() => {})
        .then(() => require("fs").promises.readFile(this.filePath, "utf8"))
  ...
  async renderMarkdownText(text, render) {
    if (!this.isCurrentRender(render)) return;
    ...
    try {
      const [domFragment, done] = await renderer.toDOMFragment(
        ...
        this.getGrammar(),
        this.editorCache,
        { signal: render.controller.signal },
      );
      ...
      if (this.isCurrentRender(render)) this.showError(error);
    }
  }
